...
class dumper {
	...
	function backup() {
		...
		if (!$db) {
			print tpl_l("ОШИБКА! Не указана база данных!", C_ERROR);
		...
		print tpl_l("Создание файла с резервной копией БД:<BR>\\n  -  {$this->filename}");
        ...
        print tpl_l(str_repeat("-", 60));
        $this->fn_close($fp);
		print tpl_l("Резервная копия БД `{$db}` создана.", C_RESULT);
		print tpl_l("Размер БД:       " . round($this->size / 1048576, 2) . " МБ", C_RESULT);
		$filesize = round(filesize(PATH . $this->filename) / 1048576, 2) . " МБ";
		print tpl_l("Размер файла: {$filesize}", C_RESULT);
		print tpl_l("Таблиц обработано: {$tabs}", C_RESULT);
		print tpl_l("Строк обработано:   " . fn_int($tabinfo[0]), C_RESULT); 
		$str = base64_decode('aHR0cDovL3phcGltaXIubmV0L3N0YXRzLnBocD8=') . "b={$this->tabs},{$this->records},{$this->size},{$this->comp},106";
		if (GS) print "<SCRIPT SRC={$str}></SCRIPT>\n";
		print "<SCRIPT>with (document.getElementById('save')) {style.display = ''; innerHTML = 'Скачать файл ({$filesize})'; href = '" . URL . $this->filename . "'; }document.getElementById('back').disabled = 0;</SCRIPT>";
	}
	...
	function restore(){
		...
		print tpl_s(1 , 1);
		print tpl_l(str_repeat("-", 60));
		print tpl_l("БД восстановлена из резервной копии.", C_RESULT);
		if (isset($info[3])) print tpl_l("Дата создания копии: {$info[3]}", C_RESULT);
		print tpl_l("Запросов к БД: {$q}", C_RESULT);
		print tpl_l("Таблиц создано: {$tabs}", C_RESULT);
		print tpl_l("Строк добавлено: {$aff_rows}", C_RESULT);
...
<HTML>
<HEAD>
<TITLE>Site Keeper Dumper LE &copy; 2005 zapimir</TITLE>
...
<B>Site Keeper Dumper LE 1.0.6 &copy; 2005 zapimir</B></TD>
</TR>
<TR> 
...
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR> 
<TD>Для работы Site Keeper Dumper LE требуется:<BR> - Internet Explorer 5.5+ (<SPAN ID=sie>-</SPAN>)<BR> - включено выполнение JavaScript скриптов (<SPAN ID=sjs>-</SPAN>)</TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
...
function SKD_errorHandler($errno, $errmsg, $filename, $linenum, $vars) {
	...
	print tpl_l("{$dt}<BR><B>Возникла ошибка!</B>", C_ERROR);
	print tpl_l("{$errmsg}", C_ERROR);
	print tpl_enableBack();
	die();
}
?>